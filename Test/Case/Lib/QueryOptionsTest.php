...
class QueryOptionsTestCase extends CakeTestCase {
    ...
    function testInit() {
        $this->assertEquals(array(), $this->options->getOptions());
    }
    ...
    function testSetOption() {
        ...
        foreach($params as $k => $v) {
            $this->assertSame($a, $a->setOption($k, $v));
        }
        $this->assertEquals($params, $a->getOptions());
    ...
    function testSetOption_Array() {
        ...
        $this->assertEquals($expected, $a->getOptions());
    ...
    function testClearOptions() {
        ...
        $this->assertSame($a, $a->clearOptions());
        $this->assertSame(array(), $a->getOptions());

        $a->setOption('key', 'value');

        $this->assertEquals(1, count($a->getOptions()));
        $this->assertSame($a, $a->clearOptions());
        $this->assertSame(array(), $a->getOptions());
    }
    ...
    function testAddOption() {
        ...
        $one = 'one is NULL';
        $this->assertSame($a, $a->addOption('conditions', $one));
    ...
    function testAddOption_Array() {
        ...
        // clears
        $a->clearOptions();
        $this->assertEquals(array(), $a->getOptions());
    ...
    function _testRemoveOption($useMethod) {
        ...
        $this->assertEquals($options, $a->getOptions());
    ...
    function testAddCondition() {
        ...
        $this->assertSame($a, $a->addCondition('id', 5));
    ...
    function testFields() {
        ...
        $a->clearOptions();
        $this->assertEquals(array('fields' => array('a')),
    ...
    function testGetAlias() {
        ...
        // always ""
        $this->assertSame("", $a->getAlias());

        $a->Alias_id(3);
        $this->assertEquals(array('id' => 3), $a->conditions);
    }
    ...
    function testIsKeyDefined_and_Call() {
        ...
        $a->recursive('a');
        $this->assertSame('a', $a->recursive);

        // custom keys
        $this->assertFalse($a->isKeyDefined('custom'));
        $this->assertSame($a, $a->custom('foo'));
        ...
        $this->assertSame('foo', $a->custom2);
        $this->assertSame($a, $a->custom2('bar'));
        $this->assertSame('bar', $a->custom2);
    }
    ...
    function testOverloadedGet() {
        ...
        $this->assertSame($fields, $a->fields);
        $this->assertSame($limit, $a->limit);
        $this->assertSame($conditions, $a->conditions);
    ...
    function testMerge() {
        ...
        $this->assertSame($a, $a->merge($b, $c));
        $afterB = $b->getOptions();
        $this->assertSame($beforeB, $afterB);

        $this->assertSame(array('a', 'b', 'c', 'd', 'e', 'f'),
                               $a->fields);
        $this->assertSame('Post', $a->jointo);
        $this->assertSame('User.id', $a->group);
        $this->assertSame(200, $a->limit);
        $this->assertSame(array('User.id' => array(1,2,3),
